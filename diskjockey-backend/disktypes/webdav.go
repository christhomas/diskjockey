...
// WebDAVDiskType implements DiskType for mounting a WebDAV server as a filesystem
// Each mount gets its own root directory on the remote server

type WebDAVDiskType struct{}
...
func (w WebDAVDiskType) New(mount *models.Mount) (types.Backend, error) {
...
func (w WebDAVDiskType) Name() string {
	return "webdav"
}

func (w WebDAVDiskType) Description() string {
	return "WebDAV remote filesystem disktype"
}

func (w WebDAVDiskType) ConfigTemplate() types.DiskTypeConfigTemplate {
	return types.DiskTypeConfigTemplate{
		"url": types.DiskTypeConfigField{
			Type:        "string",
